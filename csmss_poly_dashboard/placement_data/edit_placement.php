...
include '../common/dbcon.php'; // make sure this contains $conn

// Get ID from URL
if (isset($_GET['placement_data_id'])) {
  $id = intval($_GET['placement_data_id']);
} else {
  die("No placement ID provided.");
}
...
if (!$result || mysqli_num_rows($result) == 0) {
  die("Placement record not found.");
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $fileName = $row['pdf']; // keep old file by default

    // If a new file uploaded
    if (isset($_FILES['pdf']) && $_FILES['pdf']['error'] == 0) {
        $targetDir = "../assets/pdf/placement/placement_data/";   // ✅ FIXED PATH
        if (!is_dir($targetDir)) {
    ...
    // Update DB
    $update = "UPDATE placement_data SET title='$title', pdf='$fileName' WHERE placement_data_id=$id";
    if (mysqli_query($conn, $update)) {
        echo "<script>alert('Placement updated successfully!'); window.location='placement.php';</script>";
    } else {
  ...
  <div class="layout-wrapper layout-content-navbar">
    <div class="layout-container">
      ...
      <div class="layout-page">
        ...
        <div class="container">
          <div class="card my-4">
            <h5 class="card-header">EDIT PLACEMENT</h5>
            <div class="card-body">
                  ...
                  <div class="col-lg-6">
                    <div class="mb-3">
                      ...
                      <?php if (!empty($row['pdf'])): ?>
                        <small>
                          Current File: 
                          <a href="../uploads/placement/<?php echo $row['pdf']; ?>" target="_blank">View PDF</a>
                        </small>
                      <?php endif; ?>
                    </div>
                  </div>
